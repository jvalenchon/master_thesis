...
#import rbfopt #to do optimization
...
def run(seed, gamma, beta, hidden, lr, NB_EPOCH=300):
    ...
    for epoch in range(NB_EPOCH):
        ...
        val_avg_loss, val_classification, val_labels_corres = sess.run([model.loss, model.classification, model.labels_class], feed_dict=val_feed_dict)
...
for i in range(NB_SEEDS):
    ...
    print("seed", seed)
    hidden_a=hidden_a*23
    auc_test, auc_train, auc_val = run(seed, gamma, beta, [hidden_a, hidden_b], lr, 50)
    ...
    np.savetxt('auc_val_tadpole_seed%i.csv' %seed, auc_val, fmt='%s',delimiter=',')
    np.savetxt('auc_test_tadpole_seed%i.csv' %seed, auc_test, fmt='%s',delimiter=',')
    np.savetxt('auc_train_tadpole_seed%i.csv' %seed, auc_train, fmt='%s',delimiter=',')
