...
class Dense(Layer):
    ...
    def _call(self, inputs):
        ...
        return u_outputs, v_outputs, self.vars['weights_u'], self.vars['weights_v']
    ...
    def __call__(self, inputs):
        with tf.name_scope(self.name):
            ...
            outputs_u, outputs_v, self.vars['weights_u'], self.vars['weights_v'] = self._call(inputs)
            if self.logging:
                tf.summary.histogram(self.name + '/outputs_u', outputs_u)
                tf.summary.histogram(self.name + '/outputs_v', outputs_v)
            return outputs_u, outputs_v, self.vars['weights_u'], self.vars['weights_v']
...
class StackGCN(Layer):
    ...
    def __init__(self, input_dim, output_dim, support, support_t, num_support, u_features_nonzero=None,
                 ...
                 act=tf.nn.relu, share_user_item_weights=True, bias=False, **kwargs):
    ...
    def _call(self, inputs):
        ...
        return u_outputs, v_outputs, x_u, x_v, self.weights_u, self.weights_v
    ...
    def __call__(self, inputs):
        with tf.name_scope(self.name):
            if self.logging and not self.sparse_inputs:
                tf.summary.histogram(self.name + '/inputs_u', inputs[0])
                tf.summary.histogram(self.name + '/inputs_v', inputs[1])
            outputs_u, outputs_v, x_u, x_v, self.weights_u, self.weights_v = self._call(inputs)
            if self.logging:
                tf.summary.histogram(self.name + '/outputs_u', outputs_u)
                tf.summary.histogram(self.name + '/outputs_v', outputs_v)
            return outputs_u, outputs_v, x_u, x_v, self.weights_u, self.weights_v


class BilinearMixtureDisease(Layer):
    """
    Decoder model layer for link-prediction with disease data
    ...
    def __init__(self, num_classes, u_indices, v_indices, input_dim, num_users, num_items, user_item_bias=False,
                 dropout=0., act=tf.nn.softmax, num_weights=1,#3
                 diagonal=True, **kwargs):
        super(BilinearMixtureDisease, self).__init__(**kwargs)
        with tf.variable_scope(self.name + '_vars'):
            ...
            if user_item_bias:
                self.vars['user_bias'] = bias_variable_zero([num_users, num_weights], name='user_bias')
                self.vars['item_bias'] = bias_variable_zero([num_items, num_weights], name='item_bias')
    ...
    def _call(self, inputs):
        ...
        u_inputs =tf.nn.dropout(inputs[0], 1 - self.dropout)#inputs[0]#
        v_inputs = tf.nn.dropout(inputs[1], 1 - self.dropout)#inputs[1]#
        ...
        basis_outputs = []
        for i in range(self.num_weights):#why num weights
        ...
        outputs = basis_outputs#tf.matmul(basis_outputs,  self.vars['weights_scalars'], transpose_b=False)
        ...
        return outputs, u_inputs, v_inputs, self.vars['weights_0']
    ...
    def __call__(self, inputs):
        with tf.name_scope(self.name):
            ...
            outputs, u_inputs, v_inputs, self.vars['weights_0'] = self._call(inputs)
            if self.logging:
                tf.summary.histogram(self.name + '/outputs', outputs)
            return outputs, u_inputs, v_inputs, self.vars['weights_0']
