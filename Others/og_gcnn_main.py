    ...
    def __init__(self, M, L, A, labels_train, labels_test, labels_val, testing_set_mask, training_set_mask,validation_set_mask,
        ...
        with tf.Graph().as_default() as g:
                ...
                with tf.device(idx_gpu):
                        ...
                        bias=[]
                        if nb_layers==1:
                            weights.append(tf.get_variable('weights_W', shape=[self.ord_row*M.shape[1], self.n_conv_feat], initializer=tf.contrib.layers.xavier_initializer()))#self.n_conv_feat
                            ...
                            for i in range(nb_layers-1):
                                weights.append(tf.get_variable('weights_%d' %i, shape=[self.ord_row*self.n_conv_feat, self.n_conv_feat], initializer=tf.contrib.layers.xavier_initializer()))#self.n_conv_feat
                                bias.append(tf.Variable(tf.zeros([self.n_conv_feat,])))
                            weights.append(tf.get_variable("weight_final", shape=[self.n_conv_feat, 1], initializer=tf.contrib.layers.xavier_initializer()))
                            bias.append(tf.Variable(tf.zeros([1,])))
                        ...
                        if nb_layers==1:
                            ...
                            self.final_feat_users = tf.matmul(self.final_feat_users, weights[-1]) + bias[-1]
                        else:
                            self.final_feat_users = self.mono_conv_cheby(self.list_row_cheb_pol, self.ord_row, self.M, weights[0], bias[0]) #shape 3000*32
                            self.final_feat_users = tf.nn.relu(self.final_feat_users)
                            self.final_feat_users= tf.nn.dropout(self.final_feat_users, dropout)
                            for i in range(nb_layers-1):
...
def running_one_time(n, dropout, seed, n_conv_feat,lr, l2_regu, nb_layers, ord_row):
    ...
    """
    #initialization of the matrix, of the training, testing and validation sets#initialization
...
for seed in range(nb_seeds):
    nb_itera =500
    dropout, n_conv_feat, lr, l2_regu,nb_layers, ord_row=0.508909277, 67, 0.000678388869, 0.0000853887952, 1, 1
    n_conv_feat=int(n_conv_feat)
    nb_layers=int(nb_layers)
...
print(auc_test)
"""np.savetxt('gcnn_dense_auc_values_test_ord_row_dropout.csv', auc_test, fmt='%f', delimiter=',')
np.savetxt('gcnn_dense_auc_values_train_ord_row_dropout.csv', auc_train, fmt='%f', delimiter=',')
np.savetxt('gcnn_dense_auc_values_val_ord_row_dropout.csv', auc_val, fmt='%f', delimiter=',')
np.savetxt('gcnn_dense_pred_test_ord_row_dropout.csv', pred_test, fmt='%f', delimiter=',')
np.savetxt('gcnn_dense_pred_train_ord_row_dropout.csv', pred_train, fmt='%f', delimiter=',')
np.savetxt('gcnn_dense_pred_val_ord_row_dropout.csv', pred_val, fmt='%f', delimiter=',')
np.savetxt('gcnn_dense_labels_train_ord_row_dropout.csv', labels_train, fmt='%f', delimiter=',')
np.savetxt('gcnn_dense_labels_test_ord_row_dropout.csv', labels_test,fmt='%s',delimiter=',')
np.savetxt('gcnn_dense_labels_val_ord_row_dropout.csv', labels_val, fmt='%s',delimiter=',')"""
